...
export function createServer(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        return {
            tools: [PUBLISH_ARTICLE_SCHEMA, PREVIEW_ARTICLE_SCHEMA, LIST_THEMES_SCHEMA, REGISTER_THEME_SCHEMA, REMOVE_THEME_SCHEMA],
        };
    });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        try {
            if (request.params.name === "preview_article") {
                const args = request.params.arguments || {};
                const content = String(args.content || "");
                const themeId = String(args.theme_id || "");
                return await previewArticle(content, themeId);
            } else if (request.params.name === "publish_article") {
